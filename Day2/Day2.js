...
function day2() {
    ...
    let totalLevels = data.length;
    let safeLevels = 0;

    for (let level = 0; level < totalLevels; level++) {
        safeLevels += checkLevel(data[level]);
    }

    console.log("THE SOLUTION FOR THE 2ND DAY OF CODE ADVENT IS: " + safeLevels);
}

function checkLevel(level) {
    let levelCopy = level;
    for (let i = 0; i < level.length - 1; i++) {
        levelCopy[i] = levelCopy[i + 1] - levelCopy [i];
        if(Math.abs(levelCopy[i]) > 3) {return 0;}
        if(levelCopy[i] == 0) {return 0;}
        
        let satisfyOrderProperty = levelCopy[0] * levelCopy[i] > 0;
        if ( !satisfyOrderProperty ) {return 0;} 
    }
...
function getInputData() {
    return [[16, 17, 18, 21, 23, 24, 27, 24],
